...
def main():
    ...
    train_common_args = (
        " --quiet --eval --iterations 61000 --test_iterations -1 --save_iterations 61000"
    )
    render_common_args = " --quiet --eval --skip_train"
    render_extra_args = " --render_normals -w"
    ...
    if not args.skip_rendering:
        ...
        for idx, scene in enumerate(all_scenes, start=1):
            ...
            cmd = f"python render.py -s {shlex.quote(str(source_with_flags))} -m {shlex.quote(str(out_model_dir))}{render_common_args}"
